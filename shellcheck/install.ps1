...
IF (!(Test-Path -Path "$pkg_src_cmd"))
{
    ...
    pushd .local\tmp
        ...
        Remove-Item -Path ".\shellcheck-v*" -Recurse -ErrorAction Ignore
        Remove-Item -Path ".\shellcheck.exe" -Recurse -ErrorAction Ignore

        # NOTE: DELETE THIS COMMENT IF NOT USED
        # Move single binary into root of temporary folder
        #& move "$pkg_download" "shellcheck.exe"
        ...
        Move-Item -Path ".\shellcheck-*\shellcheck.exe" -Destination "$pkg_src_bin"
